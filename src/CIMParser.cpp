...
void CIMParser::on_characters(const Glib::ustring &characters)
{
	...
	if(is_only_whitespace(characters))
	{
 	...
 	if(!assign(elementStack.top(), tagStack.top(), replace_whitespace(characters)))
...
/**
 * convert characters to underline
 */
Glib::ustring CIMParser::replace_whitespace(const Glib::ustring &characters)
{
	Glib::ustring new_characters(characters);
	for(int i = 0; i < new_characters.length(); i++){
		if (std::isspace(new_characters.at(i)))
			new_characters = new_characters.replace(i,1,"_");
	}
