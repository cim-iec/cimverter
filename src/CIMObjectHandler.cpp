    ...
    if (auto *tp_node = dynamic_cast<TPNodePtr>(Object)) {
      ...
      for (terminal_it = tp_node->Terminal.begin(); terminal_it != tp_node->Terminal.end(); ++terminal_it) {
        ...
        if (auto *externalNI = dynamic_cast<ExNIPtr>((*terminal_it)->ConductingEquipment)) {
          ...
          if (this->configManager.household_parameters.use_households == false) {
            SolarGenerator solar_generator = this->SynchronousMachineHandlerType2(tp_node, (*terminal_it), syn_machine, dict);
      ...
      if (this->configManager.household_parameters.use_households == true) {
        ...
        while (!this->householdQueue.empty()) {
          ...
          switch (this->householdQueue.front().HouseholdType()) {
            case (HouseholdType::Type0):
              if (this->householdQueue.front().sequenceNumber() == 0 || this->householdQueue.front().sequenceNumber() == 1) {
            ...
            case (HouseholdType::Type2):
              if (this->householdQueue.front().sequenceNumber() == 0 || this->householdQueue.front().sequenceNumber() == 1) {
                ctemplate::TemplateDictionary* household_dict = dict->AddIncludeDictionary("HOUSEHOLD_TYPE2_DICT");
                household_dict->SetFilename(this->configManager.ts.directory_path + "resource/HouseholdType2.tpl");
                this->householdQueue.front().set_template_values(household_dict);
              }
              break;
            default:
        ...
        while (!this->solarGeneratorQueue.empty()) {
          ...
          ctemplate::TemplateDictionary* solar_generator_dict = dict->AddIncludeDictionary("SOLARGENERATOR_DICT");
          solar_generator_dict->SetFilename(this->configManager.ts.directory_path + "resource/SolarGenerator.tpl");
...
BusBar CIMObjectHandler::TopologicalNodeHandler(const TPNodePtr tp_node, ctemplate::TemplateDictionary* dict) {
  ...
  if (this->configManager.busbar_parameters.enable) {
    ...
    busbar.annotation.placement.transfomation.extent.first.x = configManager.busbar_parameters.annotation.transformation_extent[0];
    busbar.annotation.placement.transfomation.extent.first.y = configManager.busbar_parameters.annotation.transformation_extent[1];
    busbar.annotation.placement.transfomation.extent.second.x = configManager.busbar_parameters.annotation.transformation_extent[2];
    busbar.annotation.placement.transfomation.extent.second.y = configManager.busbar_parameters.annotation.transformation_extent[3];
  ...
  for (terminal_it = tp_node->Terminal.begin(); terminal_it != tp_node->Terminal.end(); ++terminal_it) {
    ...
    if (this->pqloadQueue.size() == 1 && this->solarGeneratorQueue.size() == 1) {
      ...
      if (this->configManager.household_parameters.enable) {
        ...
        household.annotation.placement.transfomation.extent.first.x = configManager.household_parameters.annotation.transformation_extent[0];
        household.annotation.placement.transfomation.extent.first.y = configManager.household_parameters.annotation.transformation_extent[1];
        household.annotation.placement.transfomation.extent.second.x = configManager.household_parameters.annotation.transformation_extent[2];
        household.annotation.placement.transfomation.extent.second.y = configManager.household_parameters.annotation.transformation_extent[3];
 ...
 */
ConnectivityNode CIMObjectHandler::ConnectiviyNodeHandler(const TPNodePtr tp_node, const TerminalPtr terminal, const ConnectivityNodePtr connectivity_node,
  ...
  for (diagram_it = connectivity_node->DiagramObjects.begin(); diagram_it != connectivity_node->DiagramObjects.end(); ++diagram_it) {
    _t_points = this->calculate_average_position();
...
Slack CIMObjectHandler::ExternalNIHandler(const TPNodePtr tp_node, const TerminalPtr terminal, const ExNIPtr externalNI, ctemplate::TemplateDictionary* dict) {
  ...
  if (this->configManager.slack_parameters.enable) {
    ...
    slack.annotation.placement.transfomation.extent.first.x = configManager.slack_parameters.annotation.transformation_extent[0];
    slack.annotation.placement.transfomation.extent.first.y = configManager.slack_parameters.annotation.transformation_extent[1];
    slack.annotation.placement.transfomation.extent.second.x = configManager.slack_parameters.annotation.transformation_extent[2];
    slack.annotation.placement.transfomation.extent.second.y = configManager.slack_parameters.annotation.transformation_extent[3];
 ...
 */
PiLine CIMObjectHandler::ACLineSegmentHandler(const TPNodePtr tp_node, const TerminalPtr terminal, const AcLinePtr ac_line,
                                              ctemplate::TemplateDictionary* dict) {
  PiLine Pi_line;
  ...
  if (this->configManager.piline_parameters.enable) {
    ...
    Pi_line.annotation.placement.transfomation.extent.first.x = configManager.piline_parameters.annotation.transformation_extent[0];
    Pi_line.annotation.placement.transfomation.extent.first.y = configManager.piline_parameters.annotation.transformation_extent[1];
    Pi_line.annotation.placement.transfomation.extent.second.x = configManager.piline_parameters.annotation.transformation_extent[2];
    Pi_line.annotation.placement.transfomation.extent.second.y = configManager.piline_parameters.annotation.transformation_extent[3];
 ...
 */
Transformer CIMObjectHandler::PowerTransformerHandler(const TPNodePtr tp_node, const TerminalPtr terminal, const PowerTrafoPtr power_trafo,
                                                      ctemplate::TemplateDictionary* dict) {
  ...
  if (this->configManager.trafo_parameters.enable) {
    ...
    Trafo.annotation.placement.transfomation.extent.first.x = configManager.trafo_parameters.annotation.transformation_extent[0];
    Trafo.annotation.placement.transfomation.extent.first.y = configManager.trafo_parameters.annotation.transformation_extent[1];
    Trafo.annotation.placement.transfomation.extent.second.x = configManager.trafo_parameters.annotation.transformation_extent[2];
    Trafo.annotation.placement.transfomation.extent.second.y = configManager.trafo_parameters.annotation.transformation_extent[3];
 ...
 */
PQLoad CIMObjectHandler::EnergyConsumerHandler(const TPNodePtr tp_node, const TerminalPtr terminal, const EnergyConsumerPtr energy_consumer,
                                               ctemplate::TemplateDictionary* dict) {
  ...
  if (this->configManager.pqload_parameters.use_profiles == false) {
    ...
    if (this->configManager.pqload_parameters.enable) {
      ...
      pqload.annotation.placement.transfomation.extent.first.x = configManager.pqload_parameters.annotation.transformation_extent[0];
      pqload.annotation.placement.transfomation.extent.first.y = configManager.pqload_parameters.annotation.transformation_extent[1];
      pqload.annotation.placement.transfomation.extent.second.x = configManager.pqload_parameters.annotation.transformation_extent[2];
      pqload.annotation.placement.transfomation.extent.second.y = configManager.pqload_parameters.annotation.transformation_extent[3];
      pqload.annotation.placement.visible = configManager.pqload_parameters.annotation.visible;
    }

    for (diagram_it = energy_consumer->DiagramObjects.begin(); diagram_it != energy_consumer->DiagramObjects.end(); ++diagram_it) {
 ...
 */
WindGenerator CIMObjectHandler::SynchronousMachineHandlerType1(const TPNodePtr tp_node, const TerminalPtr terminal, const SynMachinePtr syn_machine,
                                                               ctemplate::TemplateDictionary* dict) {
  ...
  if (this->configManager.wind_gen_parameters.enable) {
    ...
    wind_generator.annotation.placement.transfomation.extent.first.x = configManager.wind_gen_parameters.annotation.transformation_extent[0];
    wind_generator.annotation.placement.transfomation.extent.first.y = configManager.wind_gen_parameters.annotation.transformation_extent[1];
    wind_generator.annotation.placement.transfomation.extent.second.x = configManager.wind_gen_parameters.annotation.transformation_extent[2];
    wind_generator.annotation.placement.transfomation.extent.second.y = configManager.wind_gen_parameters.annotation.transformation_extent[3];
    wind_generator.annotation.placement.visible = configManager.wind_gen_parameters.annotation.visible;
  }

  for (diagram_it = syn_machine->DiagramObjects.begin(); diagram_it != syn_machine->DiagramObjects.end(); ++diagram_it) {
    _t_points = this->calculate_average_position();
 ...
 */
SolarGenerator CIMObjectHandler::SynchronousMachineHandlerType2(const TPNodePtr tp_node, const TerminalPtr terminal, const SynMachinePtr syn_machine,
                                                                ctemplate::TemplateDictionary* dict) {
  ...
  if (this->configManager.wind_gen_parameters.enable) {
    ...
    solar_generator.annotation.placement.transfomation.extent.first.x = configManager.wind_gen_parameters.annotation.transformation_extent[0];
    solar_generator.annotation.placement.transfomation.extent.first.y = configManager.wind_gen_parameters.annotation.transformation_extent[1];
    solar_generator.annotation.placement.transfomation.extent.second.x = configManager.wind_gen_parameters.annotation.transformation_extent[2];
    solar_generator.annotation.placement.transfomation.extent.second.y = configManager.wind_gen_parameters.annotation.transformation_extent[3];
...
DiagramObjectPoint CIMObjectHandler::calculate_average_position() {
  ...
  for (points_it = (*diagram_it)->DiagramObjectPoints.begin(); points_it != (*diagram_it)->DiagramObjectPoints.end(); ++points_it) {
